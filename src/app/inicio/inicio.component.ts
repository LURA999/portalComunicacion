...
import { imgVideoModel } from '../interfaces/img-video.model';
import { ResponseInterfaceTs } from '../interfaces/response.interface';
...
export class InicioComponent implements OnInit {
  ...
  recursoUrl(src : string, categ : boolean, n : number, sec :boolean) : SafeResourceUrl {
    if (n > 0 ) {
      if (sec === false ) {
        if (categ == false) {
